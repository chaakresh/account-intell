...
async function checkGaps(sections) {
  const summary = Object.entries(sections)
    .map(([k, v]) => `${k.toUpperCase()}: ${v.text.slice(0, 600)}`)
    .join("\n\n");
...
${summary}`;
  ...
  const raw = await claudeCall(prompt, 800);
  try {
...
async function synthesize(sections, today) {
  // Build sources map per section
  const sourcesMap = {};
  Object.entries(sections).forEach(([k, v]) => {
    sourcesMap[k] = v.sources || [];
  });

  const researchBlock = Object.entries(sections)
    .map(([k, v]) => `=== ${k.toUpperCase()} ===\n${v.text}\n\nSOURCES FOUND:\n${
      (v.sources||[]).map(s => `- ${s.name}: ${s.url}`).join("\n") || "None"
    }`)
    .join("\n\n");

  const prompt = `You are an expert IT industry analyst for ITC Infotech's CEO and leadership team.
Today: ${today}

${ITC_CONTEXT}

Based on the research below, generate a daily intelligence brief as a STRICT JSON object.

CRITICAL RULES:
1. so_what MUST name specific ITCI service line, vertical, platform, or geography — NEVER generic
2. severity: "red" = act within days, "amber" = monitor this week, "green" = awareness only
3. Sources: use the URLs provided in the research. Include 1-3 sources per signal matching the story.
4. Each section must have EXACTLY 3 signals
5. top_signals: pick the 3-5 most critical signals across all sections
6. regulatory: 3-5 items with specific deadlines
...
Return ONLY this exact JSON structure, no markdown, no explanation:
{
  ...
  "top_signals": [
    {
      ...
      "so_what": "1 sentence — specific ITCI service line/vertical/geography",
  ...
  "sections": {
    ...
    "competitive": {"title": "Competitive Intelligence", "signals": []},
    "market_structure": {"title": "Market Structure Shifts", "signals": []},
    "client_verticals": {"title": "Client Verticals", "signals": []},
    "partner_ecosystem": {"title": "Partner & Ecosystem", "signals": []}
  },
  "regulatory": {
    "last_updated": "${today}",
    "items": [
  ...
  const raw = await claudeCall(prompt, 5000);
  const clean = raw.replace(/```json|```/g, "").trim();
  const start = clean.indexOf("{");
  const end   = clean.lastIndexOf("}") + 1;
  if (start === -1 || end === 0) throw new Error("No JSON found in Claude response");
...
async function main() {
  ...
  console.log(`\n📰 Generating Daily Intelligence Brief — ${today}`);
  ...
  for (const gap of gaps) {
    ...
    try {
      const fill = await research(
        `Search specifically for: ${gap.query}. Find the most recent and relevant article. Return factual information with source URL.`,
        gap.gap
      );
  ...
  // Step 4: Claude synthesis → JSON
  process.stdout.write(`\n🧠 [Synthesizing Agent] Building intelligence brief...`);
  const report = await synthesize(sections, today);
  console.log(` ✅`);

  // Step 5: Save
  fs.writeFileSync("report.json", JSON.stringify(report, null, 2));
  const topCount = (report.top_signals || []).length;
  console.log(`\n✅ report.json saved. Top signals: ${topCount}`);
  console.log("═".repeat(60));
}
